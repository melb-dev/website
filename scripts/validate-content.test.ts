...
describe('content integrity', () => {
  ...
  it('rejects deletion', () =>
    expect(validate([], [row('events', 'old', u(2), {})]).join()).toMatch(/deletion/));
  it('rejects changed UUIDs at a stable path', () => {
    const old = row('groups', 'group', u(3), { topics: ['topic'] });
    const now = row('groups', 'group', u(4), { topics: ['topic'] });
    const topic = row('topics', 'topic', u(5), { name: 'Topic' });
    expect(validate([now, topic], [old]).join()).toMatch(/UUID cannot change/);
  });
  it('requires offset datetimes and non-empty topics', () => {
    const event = row('events', '20260101-group-test', u(6), {
      title: 'Test',
  ...
  it('requires a new revision and updated date for material changes', () => {
    ...
    const old = row('events', '20260101-group-test', u(7), data);
    const now = row('events', '20260101-group-test-renamed', u(7), {
      ...data,
      title: 'Test Renamed',
    });
